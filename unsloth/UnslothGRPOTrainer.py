...
from unsloth.config import execution_token_entropy_coef, advantage_schedule
...
def grpo_compute_loss(
    ...
    new_logits, # (1,s,v)
    old_logits, # (1,s,v)
    sampling_per_token_logps, # (1,s)
    input_ids, # (1,s)
    mask, # (1,s)
    beta, # float
    advantages, # (1)
    planning_token_mask, # (1,s)
    execution_token_mask, # (1,s)
    **kwargs
):
    ...
    input_ids = input_ids.unsqueeze(-1) # (1,s,1)
    ...
    new_x = torch.gather(new_logits, dim = -1, index = input_ids).squeeze(-1) # (1,s)
    ...
    if execution_token_mask is not None: # (1,s)
        ...
        token_entropy = -(new_probs * new_log_probs).sum(dim=-1) # (1,s)
        execution_token_mask = execution_token_mask.to(token_entropy.dtype) # (1,s)
        execution_per_token_entropy = (token_entropy * execution_token_mask).sum(dim=-1) / execution_token_mask.sum(dim=-1).clamp(min=1.0) # (1)
        planning_per_token_entropy = (token_entropy * planning_token_mask).sum(dim=-1) / planning_token_mask.sum(dim=-1).clamp(min=1.0) # (1)

        vocab_size = new_logits.size(-1)
    ...
    coef_1 =  torch.exp(log_importance_weights) # (1,s)

    coef_2 = torch.clamp(coef_1, 1 - epsilon_low, 1 + epsilon_high) # (1,s)

    # nengjanggo
    s = input_ids.size(-2)
    unsqueezed_advantages = advantages.unsqueeze(1).repeat(1, s)
    hicra_alpha = 0.2
    if advantage_schedule == 'none':
        pass
    elif advantage_schedule == 'hicra':
        unsqueezed_advantages += hicra_alpha * planning_token_mask * torch.abs(unsqueezed_advantages)
    elif advantage_schedule == 'execution_token_entropy':
        pass # TODO

    if delta is not None:
        loss_1 = torch.clamp(coef_1, max=delta) * unsqueezed_advantages
    else:
        loss_1 = coef_1 * unsqueezed_advantages
    pass
    ...
    loss_2 = coef_2 * unsqueezed_advantages
...
class UnslothEfficientGRPO(torch.autograd.Function):
    ...
    def forward(
        ctx, 
        _new_hidden_states, # (b,s,h)
        _old_hidden_states, # (b,s,h)
        _ref_hidden_states, # None if beta == 0.0
        _sampling_per_token_logps, # (b,s)
        lm_head, # (vocab_size,h)
        _input_ids, # (b,s)
        _mask, # (b,s)
        _advantages, # (b)
        _planning_token_mask, # (b,s)
        _execution_token_mask, # (b,s)
        beta, 
        scaler = None, 
        n_chunks = 1, # b
        extra_kwargs=None
        ...
        if extra_kwargs is None:
            extra_kwargs = {}
        def compute_loss(
            new_hidden_states, # (1,s,h)
            old_hidden_states, # (1,s,h)
            ...
            new_logits = torch.matmul(new_hidden_states.to(lm_head.dtype), lm_head.t()) # (1,s,v)
            ...
            loss, completion_length, mean_kl, delta, flat_is_ratio, custom_metrics = grpo_compute_loss(
                ref_logits,
                new_logits,
                old_logits,
            ...
            return scaled_loss, (loss.detach(), completion_length, mean_kl, delta, flat_is_ratio, custom_metrics)
        pass
        ...
        def accumulate_chunk(
            new_hidden_states_j, # (1,s,h)
            old_hidden_states_j, # (1,s,h)
            ref_hidden_states_j, # None
            sampling_per_token_logps_j, # (1,s)
            input_ids_j, # (1,s)
            mask_j, # (1,s)
            advantages_j, # (1)
            scaling, # float
            grad_inputs_j, # (1,s,h),
            planning_token_mask_j, # (1,s)
            execution_token_mask_j # (1,s)
        ):
            (chunk_grad_input,), (chunk_loss, (unscaled_loss, chunk_completion_length, chunk_mean_kl, chunk_delta, chunk_flat_is_ratio, chunk_custom_metrics)) = torch.func.grad_and_value(
                compute_loss,
                argnums = (0,),
                has_aux = True,
            )(new_hidden_states_j, old_hidden_states_j, ref_hidden_states_j, sampling_per_token_logps_j, input_ids_j, mask_j, advantages_j, planning_token_mask_j, execution_token_mask_j, scaling)
        ...
        if _old_hidden_states is not None: # (b,s,h)
            old_hidden_states  = torch.chunk(_old_hidden_states, chunks = n_chunks, dim = 0) # ( (1,s,h), ... , (1,s,h) )
        else: 
        ...
        if _sampling_per_token_logps is not None: # (b,s)
            sampling_per_token_logps  = torch.chunk(_sampling_per_token_logps, chunks = n_chunks, dim = 0) # ( (1,s), ... , (1,s) )
        else:
            sampling_per_token_logps = [None] * n_chunks
        input_ids          = torch.chunk(_input_ids,         chunks = n_chunks, dim = 0) # ( (1,s), ... , (1,s) )
        mask               = torch.chunk(_mask,              chunks = n_chunks, dim = 0) # ( (1,s), ... , (1,s) )
        advantages         = torch.chunk(_advantages,        chunks = n_chunks, dim = 0) # ( (1), ... , (1) )

        # nengjanggo
        planning_token_mask = torch.chunk(_planning_token_mask, chunks=n_chunks, dim=0) # ( (1,s), ... (1,s) )
        execution_token_mask = torch.chunk(_execution_token_mask, chunks=n_chunks, dim=0) # ( (1,s), ... (1,s) )
        ...
        # (1,s,h), (1,s,h), (1,s,h), None, (1,s), (1,s), (1,s), (1)
        for (grad_inputs_j, new_hidden_states_j, old_hidden_states_j, ref_hidden_states_j, sampling_per_token_logps_j, input_ids_j, mask_j, advantages_j, planning_token_mask_j, execution_token_mask_j) in \
            zip(grad_inputs_chunks, new_hidden_states, old_hidden_states, ref_hidden_states, sampling_per_token_logps, input_ids, mask, advantages, planning_token_mask, execution_token_mask):
            ...
            accumulate_chunk(
                ...
                scaling,
                grad_inputs_j,
                planning_token_mask_j,
                execution_token_mask_j
            )
        pass
...
#     # nengjanggo
#     execution_token_mask = kwargs.get('execution_token_mask', None)
#     execution_token_entropy_coef = kwargs.get('execution_token_entropy_coef', 0.0)
#     if execution_token_mask is not None:
#         new_log_probs = torch.log_softmax(new_logits, dim=-1)
#         new_probs = torch.exp(new_log_probs)

#         token_entropy = -(new_probs * new_log_probs).sum(dim=-1)
#         execution_token_mask = execution_token_mask.to(token_entropy.dtype)
#         execution_per_token_entropy = (token_entropy * execution_token_mask).sum(dim=-1) / execution_token_mask.sum(dim=-1).clamp(min=1.0)

#         vocab_size = new_logits.size(-1)
#         max_per_token_entropy = torch.log(torch.tensor(vocab_size, device=execution_per_token_entropy.device, dtype=execution_per_token_entropy.dtype))

#         normalized_execution_per_token_entropy = (execution_per_token_entropy / max_per_token_entropy).clamp(0, 1)
...
#     loss += execution_token_entropy_coef * normalized_execution_per_token_entropy
...
class _UnslothGRPOTrainer(Trainer):
    ...
    def compute_loss(
        ...
        if "train" in self._metrics:
            ...
            # nengjanggo
            self._metrics[mode]["planning_per_token_entropy"].append(planning_per_token_entropy.item())
            ...
            # nengjanggo
            self._metrics["planning_per_token_entropy"].append(planning_per_token_entropy.item())
...
class UnslothGRPOTrainer(_UnslothGRPOTrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        pass
        self.tokenized_strategic_grams = []


    # nengjanggo
    def tokenize_strategic_grams(self):
        from unsloth.strategic_grams import strategic_grams
        assert self.processing_class is not None
        for sg in strategic_grams:
            tokenized_strategic_gram = self.processing_class(sg, add_special_tokens = False, return_tensors = 'pt')['input_ids'][0].to(torch.int64)
            tokenized_strategic_gram = tokenized_strategic_gram.to(self.accelerator.device)
            self.tokenized_strategic_grams.append(tokenized_strategic_gram)


    # nengjanggo
    def get_planning_token_mask(self, completion_ids: torch.Tensor):
        if len(self.tokenized_strategic_grams) == 0:
            self.tokenize_strategic_grams()
            assert len(self.tokenized_strategic_grams) != 0
        batch_size, seq_len = completion_ids.shape
        planning_token_mask = torch.zeros((batch_size, seq_len), dtype=torch.bool, device=completion_ids.device)
        strategic_gram_counts = [0] * len(self.tokenized_strategic_grams)

        for sg_idx, tokenized_strategic_gram in enumerate(self.tokenized_strategic_grams):
            strategic_gram_len = tokenized_strategic_gram.numel()
            if strategic_gram_len > seq_len:
                continue
            for start_idx in range(0, seq_len - strategic_gram_len + 1):
                window = completion_ids[:, start_idx : start_idx + strategic_gram_len]
                match = (window == tokenized_strategic_gram.unsqueeze(0)).all(dim=-1)
                if match.any():
